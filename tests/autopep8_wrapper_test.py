...
def test_main_failing(tmpdir, input_src, expected_ret, output_src):
    ...
    assert io.open(filename, encoding='utf8').read() == output_src


@pytest.mark.usefixtures('in_tmpdir')
def test_respects_config_file():
    with io.open('setup.cfg', 'w', encoding='utf8') as setup_cfg:
        setup_cfg.write('[pep8]\nignore=E221')

    with io.open('test.py', 'w', encoding='utf8') as test_py:
